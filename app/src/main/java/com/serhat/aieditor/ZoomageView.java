    ...
    private void updateBounds(final float[] values) {
        if (getDrawable() != null) {
            bounds.set(values[Matrix.MTRANS_X],
                    ...
                    getDrawable().getIntrinsicWidth() * values[Matrix.MSCALE_X]
                            + values[Matrix.MTRANS_X],
                    getDrawable().getIntrinsicHeight() * values[Matrix.MSCALE_Y]
                            + values[Matrix.MTRANS_Y]);
        }
    }
        ...
        if (!isClickable() && isEnabled() && (zoomable || translatable)) {
            ...
            if (doubleTapToZoom && doubleTapDetected) {
                ...
                if (matrixValues[Matrix.MSCALE_X] != startValues[Matrix.MSCALE_X]) {
                    ...
                    zoomMatrix.postScale(doubleTapToZoomScaleFactor,
                            doubleTapToZoomScaleFactor, scaleDetector.getFocusX(),
    ...
    private void animateTranslationX() {
        if (getCurrentDisplayedWidth() > getWidth()) {
            ...
            //the right edge is too far to the interior
                ...
                animateMatrixIndex(Matrix.MTRANS_X,
                        bounds.left + getWidth() - bounds.right);
            }
        } else {
